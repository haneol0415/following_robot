...
class DetectPerson3d(Node):

    def __init__(self):
        super().__init__('detect_person_3d')

        self.get_logger().info('Detecting in 3D')

        self.person2d_sub  = self.create_subscription(Point, "/detected_person", self.ball_rcv_callback, 10)
        self.person3d_pub  = self.create_publisher(Point, "/detected_person_3d", 10)
        self.person_marker_pub  = self.create_publisher(Marker, "/person_3d_marker", 10)
...
def main(args=None):
    ...
    detect_person_3d = DetectPerson3d()
    
    rclpy.spin(detect_person_3d)
    
    detect_person_3d.destroy_node()
